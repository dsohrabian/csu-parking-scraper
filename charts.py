...
# Subscriptions
select_col = [field for field in cols if 'spaces' in field if 'permit' in field]
plotdf = df[select_col]
dailymins = plotdf.resample('30T').min() # resample the data to 30 minute increments. recode to whatever is best for you scheduling
...
# Public spaces
select_col = [field for field in cols if 'spaces' in field if 'public' in field]
plotdf = df[select_col]
